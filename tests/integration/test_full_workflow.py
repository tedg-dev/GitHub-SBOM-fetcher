...
class TestFullWorkflowIntegration:
    ...
    def test_successful_complete_workflow(
        self, tmp_path, mock_github_responses
    ):
        ...
        with patch("requests.Session") as mock_session_class:
            ...
            def mock_get(url, *args, **kwargs):
                ...
                if "sbom" in url and "dependency_graph" in url:
                    ...
                    response.json.return_value = mock_github_responses[
                        "dependency_sbom"
                    ]
                else:
            ...
            # Execute
            with patch.object(
                github_client, "_session", mock_session
            ):
    ...
    def test_workflow_with_failed_dependencies(
        self, tmp_path, mock_github_responses
    ):
        ...
        with patch("requests.Session") as mock_session_class:
            ...
            def mock_get_with_failures(url, *args, **kwargs):
                ...
                if "sbom" in url and "dependency_graph" in url:
                    ...
                    response.status_code = 200
                    response.json.return_value = mock_github_responses.get(
                        "npm_lodash", {}
                    )
    ...
    def test_failure_info_integration(self):
        ...
        permanent_count = sum(
            1 for f in failures if f.error_type == ErrorType.PERMANENT
        )
        transient_count = sum(
            1 for f in failures if f.error_type == ErrorType.TRANSIENT
        )
